 ...
 */
export function ProductSwatch({ category = 'roofing', brand, line, color, swatchUrl, className = '', sizes = '(max-width: 768px) 96px, 128px' }: ProductSwatchProps) {
  const catalogImageUrl = category === 'roofing' ? getProductImageUrl(brand, line, color) : null;
  const imageUrl = swatchUrl || catalogImageUrl;
  const [imgFailed, setImgFailed] = useState(false);

  const gradient = category === 'roofing' ? getShingleGradient(color) : getFrameGradient(color);
